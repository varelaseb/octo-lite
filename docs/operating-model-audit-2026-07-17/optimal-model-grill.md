...
Status: complete through Decision 105. The decision table is the chronological
...
| 47 | Minimal stream supervision | Superseded by Decision 63. Approved lightweight mechanism: one parent-owned `brief.md`, one child-owned `status.md`, safe one-line `herdr-say` notifications, and one lifecycle-bound Fable sweep while active work exists. | Current rule: one session-durable host timer invokes `operator-say sweep` for the entire operator session, including idle periods, removed only at termination or explicit handoff. No event bus, orchestration database, per-lane watcher service, or large status protocol. | Linear transitions |
...
| 71 | Minimal LLM role roster | Superseded by Decision 104 for role count. Approved: canonical LLM roles are meta-operator, orchestrator, issue-shaper, shaping-reviewer, implementer, code-reviewer, QA capture, QA reviewer, and read-only reconciler. Fix and re-review use fresh implementer and code-reviewer instances; nested stream owners reuse the orchestrator contract. | Current rule: eight roles. Issue shaping is a conditional skill the orchestrator resolves in its own persistent session, not a separate role. Acceptance, merge, evidence publication, Linear transitions, promotion, and traffic shift remain deterministic operator-authorized helpers, not LLM roles. The control panel presents gates/helpers separately. | QA model pins |
...
| 95 | Sparse Herdr operator signals | Approved: `🧠` appears only on the sole Fable operator tab. `🎤` temporarily prefixes an agent tab while the operator is interacting with that agent directly. Normal unattended tabs use no status emoji. | Keep labels as compact as possible while preserving unique human-readable identification. Epic and issue notation, field order, worker-role disambiguation, and direct-interaction lifetime remain under grill. | Herdr epic notation |
| 96 | Herdr epic marker | Approved: `◆` immediately precedes an epic identity. Normal issue identities are unmarked. | One compact exceptional marker distinguishes the coordination layer without adding decoration to every issue tab. | Herdr field order |
| 97 | Herdr label order and issue prefix | Approved: labels order the optional operator signal, compact issue and PR numbers, then the very short human-readable outcome. The presentation label omits the tracker team prefix. | Durable artifacts and receipts retain the full issue key. Exact numeric separators, worker-role disambiguation, and direct-interaction lifetime remain under grill. | Herdr numeric token |
| 98 | Herdr numeric token | Approved: the compact identity token is bare `issue/pr`, or only `issue` before a PR exists. | Omit `TUR`, `PR`, parentheses, and other presentation characters. Full tracker and PR identities remain durable elsewhere. | Herdr worker disambiguation |
...
| 103 | Prompt TDD contract | Approved: prompt, role, and skill instruction changes define observable behavior first. A fresh model session on unchanged instructions proves red only for a genuinely wrong or ambiguous decision, never a missing file, module, export, or script. A different fresh session on changed instructions proves green against the contract, then narrow adjacent regression scenarios run. Deterministic tests cover only loading, wiring, schema, invariants, and forbidden characters; they never judge prompt quality. | Never fabricate red chronology that cannot be honestly reconstructed; state the gap instead. Eval evidence stays a lean one-off scenario record. No eval service or general harness. | Role roster reconciliation |
