...
def render_codex_adapter(resolved: ResolvedRole) -> str:
    """Render a Codex CLI 0.144.6 custom-agent file.

    Codex only recognizes the documented top-level custom-agent schema
    (name, description, developer_instructions, plus optional model,
    model_reasoning_effort, sandbox_mode, mcp_servers, skills.config).
    Unsupported tables such as [adapter]/[launch]/[contract] make Codex
    reject the whole file at load time, so only supported keys appear here.
    """
    role = resolved.role
    lines = [
        "# Generated by workflows/lib/role_resolver.py. Resolve before launch. Never edit or spawn raw.",
        ...
        f"description = {_toml_string(_description(resolved.contract_text))}",
        f"model = {_toml_string(role.model)}",
        f"model_reasoning_effort = {_toml_string(role.effort)}",
        f"developer_instructions = {_toml_string(resolved.contract_text)}",
    ]
