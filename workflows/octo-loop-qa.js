...
- UPON MERGE (operator's agent, after acceptance): drop a Linear comment on the issue with the evidence-site link and the report packet (per-AC proof, edge cases/regressions checked, merge commit). Link video URLs; do not upload video files to Linear. Then regenerate downstream artifacts: python3 /root/codex-uploads/build-evidence-index.py (gallery) and, once built, the user-story catalog (Notion + spec/ copies from one generator).`
