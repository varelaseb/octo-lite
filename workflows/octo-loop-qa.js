...
- UPON MERGE (operator's agent, after acceptance): drop a Linear comment on the issue with the evidence-site link and the report packet (per-AC proof, edge cases/regressions checked, merge commit). Link video URLs; do not upload video files to Linear. In the SAME step, transition the Linear issue to Done — acceptance + merge = Done, no separate sweep (operator convention 2026-07-04). Exception: leave the issue open only when it carries explicitly-deferred remaining scope, and say why in the merge comment. Then regenerate BOTH downstream artifacts — run each command and STOP-AND-SURFACE if either errors (a silent failure here is how the story catalog drifted 82 commits / 20+ unpublished stories behind on 2026-07-04): (1) gallery — python3 /root/codex-uploads/build-evidence-index.py ; (2) user-story catalog (ONE generator -> Notion page + repo spec twin) — python3 /root/codex-uploads/build-story-catalog.py --publish-notion, then copy /root/codex-uploads/story-catalog.md to spec/user-stories.md on the user-story-catalog branch and push (refreshes PR #70's twin). Generator canonical source is octo-lite/workflows/lib/build-story-catalog.py (the codex-uploads path is a symlink); it publishes every satisfied QA verdict, so this MUST run on every acceptance or the catalog silently falls behind.`
