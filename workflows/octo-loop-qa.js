...
LINEAR STATES are lifecycle-only (operator convention 2026-07-04, 6-state consolidation 2026-07-15): the six TUR states are Backlog, Todo, In Progress, Awaiting Accept, Live, Canceled. The loop moves the issue Todo -> In Progress when implementation starts (linear issue update <ISSUE> -s "In Progress"); the QA reviewer moves it to "Awaiting Accept" when a satisfied evidence packet lands in the operator queue (that is the review-queue state for team TUR). NEVER set Live and NEVER invent states like "Accepted" — Live (the completed state, renamed from Done 2026-07-15) is reserved for the post-acceptance merge routine (acceptance ledger entry + merge = Done, in that step only).`
