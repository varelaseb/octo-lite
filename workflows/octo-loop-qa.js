...
PR body references the Linear key ("Tracks <ISSUE>"). Do NOT merge — the operator watches the QA evidence videos (see OCTO-LITE DASHBOARD) and the merge happens only after that human acceptance plus the merge-gate checks.
SPECS are law: read the relevant spec/domains/*.md decision logs before changing behavior. Return ONLY the structured result.
LINEAR STATES are lifecycle-only (operator convention 2026-07-04, 8-state model 2026-07-15): the nine TUR states are Ideas, Shaped, Todo, In Progress, Awaiting Accept, In Staging, In Preprod, Live, Canceled (Backlog is dead; Ideas=unshaped, Shaped=awaiting operator pull, Todo=loop-eligible). The loop moves the issue Todo -> In Progress when implementation starts (linear issue update <ISSUE> -s "In Progress"); the QA reviewer moves it to "Awaiting Accept" when a satisfied evidence packet lands in the operator queue (that is the review-queue state for team TUR). NEVER set In Staging, In Preprod, or Live and NEVER invent states like "Accepted" — deployment states are reserved: the post-acceptance merge routine sets In Staging (merge = staging), promote ops set In Preprod (candidate at 0%), and the traffic shift sets Live.`
